...
float const base_v = 3.3/4096;
float const base_step = 10*9.1/(9.1+47);
uint8_t const fft_num = 100;
...
//			{
//				Data[2*i]=Data[2*i]* base_v - base_step;			
//			}
    ...
    DMA_ClearITPendingBit(DMA2_Stream0, DMA_IT_HTIF0);
		...
		{	
			for(i=0;i<DATA_SIZE/4;i++)
			{
				...
				Data[4*i]=(DataReady[2*i] & 0x000FFFF);			
				Data[4*i+1] = 0;
				Data[4*i+2]=(DataReady[2*i+1] >> 16);
				Data[4*i+3] = 0; 
			}
				//t0= TIM_GetCounter(TIM3)+(u32)timeout*65536;
